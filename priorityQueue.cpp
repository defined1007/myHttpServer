...
void exch(tk_pq_t *tk_pq, size_t i, size_t j){ //exchange, 交换堆中的两个节点
    void *tmp = tk_pq->pq[i];
    tk_pq->pq[i] = tk_pq->pq[j];
    tk_pq->pq[j] = tmp;
}

void swim(tk_pq_t *tk_pq, size_t k){ //上浮，当堆中新插入一个节点时，要将其上浮到一个正确的位置
...
int sink(tk_pq_t *tk_pq, size_t k){ //下沉，当删除堆中最小的节点后，要调整堆
    size_t j;
    size_t nalloc = tk_pq->nalloc;
    while((k << 1) <= nalloc){ //k<<1，即k*2
        j = k << 1;
        if((j < nalloc) && (tk_pq->comp(tk_pq->pq[j+1], tk_pq->pq[j]))) //选取pq[j]和pq[j+1]中较小的节点
            j++;
...
int tk_pq_init(tk_pq_t *tk_pq, tk_pq_comparator_pt comp, size_t size){
    ...
    tk_pq->pq = (void **)malloc(sizeof(void *) * (size + 1)); //size+1, 说明堆存在哨兵节点， 堆中的数据节点下标从1开始，这样对堆操作比较方便
    if (!tk_pq->pq)
        return -1;
...
void *tk_pq_min(tk_pq_t *tk_pq){
    ...
    return tk_pq->pq[1]; // 堆存在哨兵节点，故堆的最小节点是下标为1的节点
}
...
int tk_pq_delmin(tk_pq_t *tk_pq){
    ...
    --tk_pq->nalloc;
    sink(tk_pq, 1);
    if((tk_pq->nalloc > 0) && (tk_pq->nalloc <= (tk_pq->size - 1)/4)){ //减小堆
...
int tk_pq_insert(tk_pq_t *tk_pq, void *item){
    if(tk_pq->nalloc + 1 == tk_pq->size){
        if(resize(tk_pq, tk_pq->size * 2) < 0){ //扩大堆
            return -1;
        }
    }
